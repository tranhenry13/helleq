...
function UserInfo() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("https://api.lanyard.rest/v1/users/1261118171222380674");
  ...
  const fetchWeather = () => {
    axios
      .get("https://api.openweathermap.org/data/2.5/weather?q=Vinh,vn&appid=a601622a383aee1aea5573743d8e8875&units=metric")
      .then((response) => {
        ...
        <div className='md:flex gap-6 items-center'>
          ...
          <div className='text-gray-900 '>
            <h2 className='font-semibold text-2xl'>
              Hey, I'm{" "}
              <Tippy animation='scale' content='Lệ Quyên'>
                <span className='text-cyan-800'>Quyên</span>
              </Tippy>{" "}
              👋
            </h2>
            <div className='w-full h-1 bg-cyan-500 rounded-sm my-1'></div>
            <div className='font-semibold text-justify'>
              Chào mọi người! Tớ là Quyên (<span className='text-cyan-800'>200x</span>), một cô gái đam mê các tựa game trong roblox, hay làm nũng, tớ rất mong kiếm được thật nhiều bạn bè ❤️.{" "}
    ...
    <>
      <div className='md:flex gap-6 items-center'>
        ...
        <div className='text-gray-900 '>
          ...
          <div className='w-full h-1 bg-cyan-500 rounded-sm my-1'></div>
          <div className='font-semibold text-justify'>
            Chào mọi người! Tui là Quyên (<span className='text-cyan-600'>200x</span>), một cô gái đam mê các tựa game trong roblox, hay làm nũng, tui rất mong kiếm được thật nhiều bạn bè ❤️.{" "}
            <Link className='text-slate-600 underline' to='/skills'>
              Xem thêm không nhỉ
            </Link>{" "}
            ✒️
